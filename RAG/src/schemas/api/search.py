...
class HybridSearchRequest(BaseModel):
    ...
    knowledge_source: str = Field("nuke", description="Knowledge base to search: 'nuke'")

    class Config:
        populate_by_name = True
        json_schema_extra = {
            "example": {
                "query": "how to blur in nuke",
                "size": 10,
                "latest_papers": False,
                "use_hybrid": True,
            }
...
class SearchHit(BaseModel):
    """Individual search result from Nuke documentation."""
    ...
    # Nuke docs fields
    url: Optional[str] = Field(None, description="Source URL for Nuke documentation page")
    nuke_node_name: Optional[str] = Field(None, description="Nuke node name (e.g. 'Blur')")
    section: Optional[str] = Field(None, description="Nuke documentation section")
