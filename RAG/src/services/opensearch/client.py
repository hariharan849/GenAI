...
from .index_config_hybrid import DOCS_CHUNKS_MAPPING, HYBRID_RRF_PIPELINE
...
class OpenSearchClient:
    ...
    def _create_hybrid_index(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if not self.client.indices.exists(index=self.index_name):
                self.client.indices.create(index=self.index_name, body=DOCS_CHUNKS_MAPPING)
    ...
    def search_unified(
        ...
        :param knowledge_source: Knowledge base to search (selects the target index)
        ...
        try:
            ...
            if not query_embedding or not use_hybrid:
                return self._search_bm25_only(
                    query=query, size=size, from_=from_, categories=categories, latest=latest, index_name=index_name
                )
