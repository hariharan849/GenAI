...
## Corpus Check
- 226 files · ~63,857 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1345 nodes · 2701 edges · 107 communities (92 shown, 15 thin omitted)
...
- [[_COMMUNITY_ChunkIndexPayload|ChunkIndexPayload]]
- [[_COMMUNITY_Domain Context|Domain Context]]
- [[_COMMUNITY_TestChunkSections|TestChunkSections]]
...
Cohesion: 0.06
Nodes (86): # NOTE: top_k is baked in here as a closure constant at service-init, # IMPORTANT: CallbackHandler automatically inherits the current span context, GraphConfig, Configuration for the entire graph execution.      This is the configuration use, Context, Runtime context for agent dependencies.      This contains immutable dependencie, _build_local_graph(), Local LangGraph dev-server entrypoint.  This module is intentionally separate fr (+78 more)

### Community 1 - "BaseModel"
Cohesion: 0.17
Nodes (18): hybrid_search(), EmbeddingsDep, SearchDep, Hybrid search endpoint supporting multiple search modes., HealthResponse, Health check response model., Individual service status., ServiceStatus (+10 more)

### Community 2 - "OpenSearchClient"
Cohesion: 0.05
Nodes (35): OpenSearch search backend exports., OpenSearchClient, Any, Unified OpenSearch client supporting both simple BM25 and hybrid search., Create RRF search pipeline for native hybrid search.          :param force: If T, BM25 search for papers., Pure vector search on chunks.          :param query_embedding: Query embedding v, OpenSearch client supporting BM25 and hybrid search with native RRF. (+27 more)

### Community 3 - "OllamaClient"
Cohesion: 0.06
Nodes (44): ConfigurationError, LLMException, OllamaConnectionError, OllamaException, OllamaTimeoutError, OpenSearchException, Base exception for LLM-related errors., Exception raised for Ollama service errors. (+36 more)

### Community 4 - "extract_triples"
Cohesion: 0.25
Nodes (17): extract_triples(), Extract graph triples from Nuke docs.      Deterministic parsing handles the Nuk, _fact(), Unit tests for api.services.graph.extraction., _result(), test_extract_triples_empty_text(), test_extract_triples_empty_token_alignment_error_is_no_result(), test_extract_triples_happy_path_typed_facts() (+9 more)

### Community 5 - "get_settings"
Cohesion: 0.16
Nodes (19): main(), _print_score_table(), CaseResult, Run the eval harness end to end against /ask-agentic and print + persist results, lifespan(), _load_graph_client(), _load_known_nodes(), FastAPI (+11 more)

### Community 6 - "NukePageRepository"
Cohesion: 0.12
Nodes (8): Update a record by ID., NukePage, NukePageRepository, Session, TestGetUnindexedPages, TestKgState, TestMarkIndexed, TestUpsertPagesStoresSections
...
Cohesion: 0.14
Nodes (20): get_agentic_rag_service(), get_cache_client(), get_embeddings_service(), get_langfuse_tracer(), get_ollama_client(), get_opensearch_client(), get_request_settings(), get_search_client() (+12 more)
...
Cohesion: 0.16
Nodes (18): ask_question(), ask_question_stream(), _extract_sources(), _generate_query_embedding(), _prepare_chunks_and_sources(), DatabaseDep, EmbeddingsDep, LangfuseDep (+10 more)

### Community 12 - "AskRequest"
Cohesion: 0.18
Nodes (9): NukeDocChunk, _compute_minhash(), _extract_kg_for_indexed_pages(), _normalize_page_ids(), Extract KG triples for indexed pages that have not already been processed., Base, MinHash, Tests for NukePageRepository — upsert, get_unindexed, mark_indexed. (+1 more)
...
Cohesion: 0.15
Nodes (11): _decode(), Any, Redis, Redis Stack-backed semantic cache for final RAG answers.  The cache is deliberat, Final-answer semantic cache using Redis Stack vector search., Answer-shaping fields that must match before a cached answer is valid., SemanticCacheBypass, SemanticCacheClient (+3 more)

### Community 15 - "indexing.py"
Cohesion: 0.16
Nodes (16): _chunk_page_remote(), _embed_batch_remote(), index_nuke_docs_ray(), _os_bulk_remote(), Ray Data flat_map operator: 1 page dict → N chunk dicts., Ray Data map_batches operator: add embedding column to a chunk batch.      Ray D, Ray Data map_batches operator: bulk index embedded chunks into the configured se, Index unindexed Nuke pages from PostgreSQL into OpenSearch via Ray Data.      Ra (+8 more)

### Community 16 - "BaseDatabase"
Cohesion: 0.05
Nodes (33): ABC, BaseDatabase, BaseRepository, Any, Session, Initialize the database connection., Close the database connection., Get a database session. (+25 more)
...
Cohesion: 0.18
Nodes (11): ChunkMetadata, A chunk of text with metadata., Metadata for a text chunk., TextChunk, Service for chunking text into overlapping segments.      Uses word-based chunki, Chunk a list of sections, preserving section boundaries.          Each section i, Initialize text chunker.          :param chunk_size: Target number of words per, Split text into words while preserving whitespace information. (+3 more)
...
Cohesion: 0.16
Nodes (15): get_database(), get_db_session(), Get or create database instance., Get a database session context manager., make_database(), Factory function to create a database instance.      :returns: An instance of th, _doc_id(), index_nuke_docs() (+7 more)
...
Cohesion: 0.08
Nodes (14): Observability helpers., Tracing helper exports., RAGTracer, Simple, efficient Langfuse tracing utility for RAG pipeline., End generation span with response., Clean, purpose-built tracer for RAG operations., End main request trace., Main request trace context manager. (+6 more)
...
### Community 25 - "ask.py"
Cohesion: 0.17
Nodes (20): RedisSettings, AskResponse, Response model for RAG question answering., CacheClient, Redis, Redis-based exact match cache for RAG queries., Generate exact cache key based on request parameters., Find cached response for exact query match. (+12 more)
...
### Community 27 - "ask.py"
Cohesion: 0.16
Nodes (15): ask_agentic(), AgenticRAGDep, LangfuseDep, SemanticCacheDep, SessionDep, Agentic RAG endpoint with intelligent retrieval and query refinement.      Featu, Submit user feedback for an agentic RAG response.      This endpoint allows user, submit_feedback() (+7 more)
...
Cohesion: 0.07
Nodes (30): SearchSettings, make_search_client(), make_search_client_fresh(), SearchClient, Factory for the configured search backend., Search domain interfaces and backend factories., deterministic_chunk_id(), PostgresEmbeddingSearchClient (+22 more)
...
Cohesion: 0.30
Nodes (10): RAGInteraction, _build_user_metadata(), Any, Session, RAGInteractionRepository, record_rag_interaction(), AskRequest, Request model for RAG question answering. (+2 more)
...
Cohesion: 0.15
Nodes (7): Knowledge graph client exports., _fact_cypher(), Neo4jClient, Document, Merge typed graph facts into Neo4j. Returns facts written., Async Neo4j client for knowledge graph retrieval., Return 1-hop neighbours of a NukeNode as Documents.
...
Cohesion: 0.18
Nodes (8): metrics_endpoint(), Response, configure_event_loop_policy(), configure_prometheus_registry(), Runtime process configuration that must happen before the app is built., { handleRequest }, openai, serviceAdapter
...
### Community 61 - "TODOS"
Cohesion: 0.27
Nodes (11): _dedupe_triples(), _extract_control_triples(), _extract_deterministic_triples(), _extract_input_triples(), _extract_section(), _extraction_to_triple(), _input_object_name(), _operation_name() (+3 more)
...
Cohesion: 0.18
Nodes (6): KGExtractionError, Raised when the provider path fails and the caller wants retry semantics., RuntimeError, Tests for explicit knowledge graph ingestion., test_extract_kg_for_indexed_pages_keeps_failed_pages_pending(), test_extract_kg_for_indexed_pages_marks_successful_pages()

### Community 88 - "nuke_docs_ingestion.py"
Cohesion: 0.18
Nodes (5): cleanup_nuke_temp(), Delete the temp file written by scrape_nuke_docs, plus any orphaned files older, Airflow compatibility wrapper for KG ingestion tasks., generate_nuke_report(), Log a summary of the Nuke docs ingestion run.

### Community 89 - "extraction.py"
Cohesion: 0.20
Nodes (4): Knowledge graph ingestion task helpers., Knowledge graph triple extraction exports., Knowledge graph ingestion exports., Knowledge graph domain package.

### Community 90 - "nuke_ingestion.py"
Cohesion: 0.33
...
Cohesion: 0.43
Nodes (6): get_settings(), make_embeddings_client(), Factory function to create embeddings client.      Creates a new client instance, make_hybrid_indexing_service(), Factory function to create hybrid indexing service.      Creates a new service i, HybridIndexingService

### Community 94 - "test_neo4j_client.py"
Cohesion: 0.29
Nodes (3): Unit tests for Neo4jClient.write_triples()., test_write_triples_happy_path(), test_write_triples_uses_typed_labels_and_predicate()

### Community 95 - "make_neo4j_client"
Cohesion: 0.47
Nodes (3): Knowledge graph factory exports., make_neo4j_client(), Return a Neo4jClient if Neo4j is enabled in settings, else None.
...
### Community 98 - ".get_callback_handler"
Cohesion: 0.40
Nodes (5): health_check(), DatabaseDep, SearchDep, SettingsDep, Comprehensive health check endpoint for monitoring and load balancer probes.

### Community 99 - "ChunkIndexPayload"
Cohesion: 0.50
...
- **Why does `Settings` connect `Settings` to `AgentState`, `OpenSearchClient`, `OllamaClient`, `get_settings`, `LangfuseTracer`, `dependencies.py`, `SemanticCacheClient`, `config.py`, `ask.py`, `factory.py`, `Neo4jClient`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `LangfuseTracer` connect `LangfuseTracer` to `AgentState`, `OllamaClient`, `get_settings`, `dependencies.py`, `RAGTracer`, `Settings`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `OllamaClient` connect `OllamaClient` to `AgentState`, `BaseModel`, `.get_callback_handler`, `get_settings`, `dependencies.py`, `Settings`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Are the 12 inferred relationships involving `Settings` (e.g. with `PostgresEmbeddingSearchClient` and `GraphConfig`) actually correct?**
