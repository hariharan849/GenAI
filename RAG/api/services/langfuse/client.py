...
class FallbackPrompt:
    """Mirrors the Langfuse prompt object API when Langfuse is unavailable."""
...
class LangfuseTracer:
    ...
    def trace_rag_request(
        ...
        """Create a top-level trace for a RAG request."""
        if not self.client:
            yield None
            return
        try:
    ...
    def create_span(self, trace, name: str, input_data: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None):
        """Create a child span on an existing trace. Returns the span or None."""
    ...
    def fetch_prompt(self, name: str, fallback_template: str):
        """Fetch a versioned prompt from Langfuse.

        Returns the Langfuse prompt object (with .compile()) or a FallbackPrompt
        using the hardcoded template when Langfuse is disabled or unavailable.
        Pass the returned object to start_generation(prompt=...) to link the
        prompt version to the generation span in traces.
        """
        if not self.client:
