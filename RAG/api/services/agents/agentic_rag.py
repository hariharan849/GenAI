...
class AgenticRAGService:
    ...
    Memory note: ``user_id`` (see :meth:`ask`) is a caller-supplied identifier,
    not an authenticated identity — this project has no auth layer. Conversation
    memory (checkpointer thread + any future Graphiti episodes) is keyed on it
    as a single-trusted-user convenience, not a multi-tenant guarantee. Do not
    expose this service to untrusted callers before adding real authentication.
    """
    ...
    def __init__(
        ...
        :param checkpointer: Optional LangGraph checkpoint saver for thread-scoped
    ...
    async def ask(
        ...
        :param user_id: Caller-supplied identifier for tracing and conversation
            memory. Not authenticated (see class docstring).
        :param session_id: Optional identifier to start a fresh memory thread for
            this user. Omit to continue the user's single default thread.
        ...
        async def _execute_with_trace():
            ...
            if trace is not None:
                with trace as trace_obj:
                    ...
                    return await self._run_workflow(query, model_to_use, user_id, session_id, trace_obj)
            else:
                return await self._run_workflow(query, model_to_use, user_id, session_id, None)
    ...
    async def _run_workflow(
        self, query: str, model_to_use: str, user_id: str, session_id: Optional[str], trace
    ) -> dict:
        ...
        try:
            ...
            runtime_context = Context(
                ...
                top_k=self.graph_config.top_k,
                retrieval_top_k=self.graph_config.retrieval_top_k,
            ...
            # LangGraph reads thread_id from config["configurable"], not the top
            # level — a prior version of this code set it at the top level, which
            # LangGraph silently ignored, making the checkpointer a no-op even with
            # a stable id. Omitting session_id keeps one continuous thread per user;
            # passing a session_id starts a fresh thread on demand.
            thread_id = f"user_{user_id}:{session_id or 'default'}"
            ...
            retrieval_context = self._extract_retrieval_context(result)
            ...
            return {
                ...
                "output_guardrail_score": (
                    result.get("output_guardrail_result").score if result.get("output_guardrail_result") else None
                ),
                "rejected_at": self._compute_rejected_at(result),
    ...
    def _compute_rejected_at(self, result: dict) -> Optional[str]:
        """Tell API consumers which guardrail layer rejected the request, if any.

        :param result: Raw graph result dict
        :returns: "input" | "output" | None
        """
        guardrail_result = result.get("guardrail_result")
        if guardrail_result and guardrail_result.score < self.graph_config.guardrail_threshold:
            return "input"

        output_guardrail_result = result.get("output_guardrail_result")
        if output_guardrail_result and output_guardrail_result.score == 0:
            return "output"

        return None

    def _extract_retrieval_context(self, result: dict) -> List[str]:
        """Extract retrieved chunk text from graph result, for eval/RAG-metric use.

        Reads the structured ``retrieved_documents`` field (post-rerank) rather
        than the flattened tool-message string, so DeepEval's contextual
        metrics see individual chunks instead of one merged blob.
        """
        documents = result.get("retrieved_documents") or []
        return [doc.page_content for doc in documents if getattr(doc, "page_content", None)]
    ...
    def _extract_reasoning_steps(self, result: dict) -> List[str]:
        ...
        if output_guardrail_result:
            steps.append(f"Output guardrail check (score: {output_guardrail_result.score}/100)")
