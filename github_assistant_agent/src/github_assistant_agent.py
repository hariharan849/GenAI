...
class GitHubAssistantFlow(Flow[GitHubAssistantState]):
    ...
    def search(self, event: str) -> str:
        ...
        react_agent = Agent(
            ...
            tools=[github_tool],
            verbose=True,
            max_iter=5,   # GitHub Search API: 30 req/min authenticated; keep iterations low
            max_rpm=10,   # stay well under the search-API per-minute cap
        ...
        try:
            ...
            if "403" in msg or "rate limit" in msg.lower():
                self.state.error = (
                    ...
                    "The Search API allows 30 req/min for authenticated tokens. "
                    "Wait a minute and retry, or verify your GITHUB_TOKEN is valid."
                )
            else:
