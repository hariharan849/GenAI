...
class GraphConfig(BaseModel):
    ...
    :param retrieval_top_k: Wide candidate count fed into the tool before
        reranking (distinct from top_k, which is the legacy/unused-by-tool
        field once retrieval_top_k takes over the tool's candidate count)
    :param rerank_top_k: Final candidate count kept after reranking, the
        number actually passed on to grading/generation
